...
object Infer {
  ...
  def new_var[A](level:level):Ty[A] = TVar(Unbound[A](next_id(), level))
  def new_gen_var[A]():Ty[A] = TVar(Generic[A](next_id()))
  ...
  def match_fun_ty(num_params:Int, ty:t_ty):(List[t_refined_ty],t_refined_ty) = {
    ...
    case tvar@TVar(Unbound(id, level)) =>
        val param_r_ty_list = List.fill(num_params){Plain(new_var[t_expr](level))}
  ...
  def infer_ty(env:Env.env, level:level, ty:Ty[s_expr]):t_ty = {
    ty match {
      ...
      case TVar(Link(ty)) => infer_ty(env, level, ty)
    }
  }

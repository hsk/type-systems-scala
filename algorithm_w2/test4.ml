...
and tv =
  ...
  | Link of typ
and levels = {mutable level_new : level}

let rec print_typ = function
  | TVar({contents=v}) -> "TVar({contents="^print_tv v^"})"
  | TArrow(t1,t2,{level_new}) ->
    Printf.sprintf "TArrow(%s, %s, {%d})" (print_typ t1) (print_typ t2) level_new
...
let new_arrow : level -> typ -> typ -> typ = fun level ty1 ty2 ->
  TArrow(ty1,ty2,{level_new = level})
...
let rec occurs : tv ref -> typ -> unit = fun tvr -> function
  ...
  | TArrow (t1,t2,ls) ->
      let min_level = 
        (match !tvr with Unbound (_,l) -> min l ls.level_new | _ -> ls.level_new) in
      ls.level_new <- min_level;
    occurs tvr (repr t1); occurs tvr (repr t2)
  | _ -> assert false
...
let rec unify : typ -> typ -> unit = fun t1 t2 ->
  ...
  | (TArrow (tyl1,tyl2,l1), TArrow (tyr1,tyr2,l2)) ->
      unify tyl1 tyr1;
      unify tyl2 tyr2
  | _ -> assert false
...
let rec generalize level typ: typ = 
  ...
  | TArrow (ty1,ty2,l) when l.level_new > level ->
    new_arrow generic_level (generalize level ty1) (generalize level ty2)
  | ty -> ty
...
let instantiate (level:level)(ty:typ) : typ = 
  let rec loop subst = function
    ...
    | TArrow (ty1,ty2,l) when l.level_new = generic_level -> 
...
let _ =
  ...
  let l = {level_new=1} in
  let top_type_check : exp -> typ = fun exp ->
    reset_gensym ();
    infer 1 [] exp
  in
  let test(t1,t2) =
    if (t1 <> t2) then Printf.printf "error %s" (print_typ t2);
    t1 = t2
  in
  assert(test(
    TArrow (TVar {contents = Unbound ("a", 1)},
     TVar {contents = Unbound ("a", 1)}, l)
      , 
      top_type_check id));
       ...
       {contents =
         Link
          (TArrow (TVar {contents = Unbound ("b", 1)},
            TVar {contents = Unbound ("c", 1)}, l))},
     TArrow (TVar {contents = Unbound ("b", 1)},
      TVar {contents = Unbound ("c", 1)}, l), l)
     ,
       top_type_check c1));
  ...
  assert(test(
    TArrow
     (TArrow (TVar {contents = Unbound ("d", 1)},
       TVar {contents = Unbound ("e", 1)}, l),
     TArrow (TVar {contents = Unbound ("d", 1)},
      TVar {contents = Unbound ("e", 1)}, l), l)
     ,
     top_type_check (Let ("x",c1,Var"x"))));

  assert(test(
    TArrow (TVar {contents = Unbound ("b", 1)},
     TVar {contents = Unbound ("b", 1)}, l)
     ,
      ...
      {contents =
        Link
         (TArrow (TVar {contents = Unbound ("d", 1)},
           TVar {contents = Unbound ("e", 1)}, l))},
     TArrow (TVar {contents = Unbound ("d", 1)},
     TVar {contents = Unbound ("e", 1)}, l), l), l)
   ,

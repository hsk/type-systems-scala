...
let rec print_typ = function
  ...
  | TArrow(t1,t2,{level_old;level_new}) ->
    Printf.sprintf "TArrow(%s, %s, {%d, %d})" (print_typ t1) (print_typ t2) level_old level_new
  ...
  let update_level (level:level) (typ:typ): unit =
    ...
    | TArrow (_,_,ls) as ty -> 
        ...
        if level < ls.level_new then begin
          ...
          if ls.level_new = ls.level_old then begin
            (*遅延用リストに加える*)
...
let _ =
  ...
  in
  let test(typ, exp) =
    let ty1 = top_type_check exp in
    if (ty1 <> typ) then
      Printf.printf "ty1 = %s" (print_typ ty1);
    assert(ty1 = typ)
  in
  ...
  test(
    TArrow
     ...
     TArrow
      ...
      {level_old = 1; level_new = 1}),
     {level_old = 1; level_new = 1})
     , (Fun ("x", Fun ("y",Let ("x",Call (Var"x",Var"y"),
                                      Fun ("x",Call (Var"y",Var"x")))))));

(*
  TArrow(TVar({contents=
    Link(TArrow(TVar({contents=Link(
      TArrow(TVar({contents=Unbound("d",1)}),
        TVar({contents=Unbound("e",1)}), {1, 1}))}),
    TVar({contents=Unbound("c",100000000)}), {2, 1}))}),
  TArrow(TVar({contents=Link(TArrow(TVar({contents=Unbound("d",1)}),
    TVar({contents=Unbound("e",1)}), {1, 1}))}), TArrow(TVar({contents=Unbound("d",1)}), TVar({contents=Unbound("e",1)}), {1, 1}), {1, 1}), {1, 1})s
*)
